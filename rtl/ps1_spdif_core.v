module ps1_spdif_core #(
    ...
    ps1_pcm_rx #(.LRCK_LEFT(LRCK_LEFT), .SAMPLE_ON_NEGEDGE(SAMPLE_ON_NEGEDGE)) rx (
        .rst_n(rst_n), .bclk(bclk), .lrck(lrck), .sdata(sdata),
        .frame(rx_frame), .frame_valid(rx_valid));

    async_fifo fifo (
        .wr_clk(bclk), .wr_rst_n(rst_n), .wr_data(rx_frame),
        .wr_en(rx_valid), .wr_full(fifo_full),
        .rd_clk(clk_384fs), .rd_rst_n(rst_n), .rd_data(fifo_data),
        .rd_en(sample_request && !fifo_empty), .rd_empty(fifo_empty));

    spdif_tx tx (.clk_384fs(clk_384fs), .rst_n(rst_n),
    ...
    always @(posedge clk_384fs or negedge rst_n)
        if (!rst_n) begin
            underflow_q <= 1'b0;
